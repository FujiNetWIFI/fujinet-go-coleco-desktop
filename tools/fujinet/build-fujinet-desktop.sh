...
# for the Bally ColecoVision desktop app.
#
...
#
# fnconfig.ini is forced to [BOIP] enabled=1 host=127.0.0.1 port=11502
# (COLECOSESSION_BOIP_PORT). Note what that means for THIS target: the RS232
...
# in-process emulator's FujiNet cartridge (core/coleco/fujinet_cart.c, over
# the staged fujitcp) connects out to it as a plain TCP client. Same
...
# adamcore has no chdir() of its own today, but the rule below is what keeps
...
TARGET_KEY_FILE="${CLONE_DIR}/build/.coleco-target"
...
echo "FujiNet ColecoVision desktop runtime outputs:"
