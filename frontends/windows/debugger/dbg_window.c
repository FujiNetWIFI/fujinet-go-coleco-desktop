...
typedef struct {
    ...
    uint8_t pal_rgba[COLECOVDP_PAL_W * COLECOVDP_PAL_H * 4];
    uint8_t nt_bgra[256 * 192 * 4], pat_bgra[256 * 64 * 4];
    uint8_t spr_bgra[128 * 64 * 4];
    uint8_t pal_bgra[COLECOVDP_PAL_W * COLECOVDP_PAL_H * 4];
...
static int parse_addr(debugger *d, const char *text, uint16_t *out)
{
    ...
    if (colecodebug_symbol_find(d->dbg, text, &addr)) {
        *out = addr;
        return 1;
    }
    return 0;
...
static void refresh_disasm(debugger *d)
{
    ...
    n = colecodebug_disassemble(d->dbg, d->disasm_base, DISASM_LINES, lines);
    for (i = 0; i < n; i++) {
        char bytes[16] = "";
        int b;
        if (lines[i].symbol)
        ...
        len += (size_t)snprintf(
            ...
            colecodebug_bp_is_set(d->dbg, lines[i].addr) ? '*' : ' ',
...
static void refresh_mem(debugger *d)
{
    ...
    colecodebug_read_mem(d->dbg, d->mem_base, data, sizeof(data));
    text[0] = '\0';
...
static void refresh_bps(debugger *d)
{
    ...
    n = colecodebug_bp_list(d->dbg, bps, 128);
    text[0] = '\0';
    if (!n)
        snprintf(text, sizeof(text), "(no breakpoints)\r\n");
    for (i = 0; i < n; i++) {
        uint16_t off = 0;
        const char *sym = colecodebug_symbol_at(d->dbg, bps[i], &off);
        if (sym)
            len += (size_t)snprintf(text + len, sizeof(text) - len,
                                    "%04X  %s+%X\r\n", bps[i], sym, off);
        else
...
{
    static colecotrace_entry entries[TRACE_ROWS];
    char *text;
    size_t cap = TRACE_ROWS * 48 + 64;
    size_t len = 0;
    int i, n;

    n = colecodebug_trace_read(d->dbg, entries, TRACE_ROWS);
    text = (char *)malloc(cap);
    if (!text)
        return;
...
static void refresh_vdp(debugger *d)
{
    ...
    colecodebug_vdp_snapshot(d->dbg, &d->snap);

    colecovdp_render_nametable(&d->snap, d->nt_rgba);
    rgba_to_bgra(d->nt_rgba, d->nt_bgra, 256 * 192);

    bank = (int)SendMessageA(d->pat_bank, CB_GETCURSEL, 0, 0);
    if (bank < 0)
        bank = 0;
    colecovdp_render_patterns(&d->snap, bank, d->pat_rgba);
    rgba_to_bgra(d->pat_rgba, d->pat_bgra, 256 * 64);

    colecovdp_render_sprites(&d->snap, d->spr_rgba, info);
    rgba_to_bgra(d->spr_rgba, d->spr_bgra, 128 * 64);

    colecovdp_render_palette(&d->snap, d->pal_rgba);
    rgba_to_bgra(d->pal_rgba, d->pal_bgra, COLECOVDP_PAL_W * COLECOVDP_PAL_H);
...

    {
        ...
        colecovdp_format_state(&d->snap, state, (int)sizeof(state));
        set_text_lf(d->vdp_state, state);
        colecovdp_format_hex(&d->snap, d->vram_base, VRAM_ROWS, state,
                           (int)sizeof(state));
...
static void refresh_all(debugger *d)
{
    ...
    refresh_trace(d);
    refresh_vdp(d);
    set_text(d->pause_btn, colecodebug_is_paused(d->dbg) ? "Continue (F5)"
                                                       : "Pause (F5)");
}
...
{
    if (colecodebug_is_paused(d->dbg)) {
        colecodebug_resume(d->dbg);
        set_text(d->status, "Running");
        refresh_all(d);
    } else {
        colecodebug_pause(d->dbg);
    }
}
...
static void on_accept(debugger *d, int id)
{
    ...
    case IDC_VRAM_ADDR: {
        ...
        n = colecovdp_parse_poke(buf, &addr, bytes, POKE_MAX);
        if (n < 0) {
        ...
        if (n > 0) {
            colecodebug_vdp_write(d->dbg, addr, bytes, n);
...
static void on_stopped(debugger *d, colecodebug_stop_reason reason, uint16_t pc)
{
    static const char *const names[] = {"paused", "breakpoint", "step",
                                        "run-to"};
    char buf[160];
    uint16_t off = 0;
    const char *sym = colecodebug_symbol_at(d->dbg, pc, &off);
        ...
        *(WORD *)buf = (WORD)(sizeof(buf) - 1);
        ...
        if (n >= 6) {
            ...
            if (sscanf(buf + 2, "%4X", &addr) == 1 && addr <= 0xFFFF) {
                colecodebug_bp_toggle(g_dbg->dbg, (uint16_t)addr);
...
static HWND pixels(debugger *d, int id, pixel_view *pv, const uint8_t *bgra,
                   int w, int h)
{
    HWND view = child(d, "ColecoDbgPixels", "", 0, id);
    pv->bgra = bgra;
    pv->w = w;
    pv->h = h;
...
static void build_controls(debugger *d)
{
    ...
    d->view_pal = pixels(d, IDC_VIEW_PAL, &d->pal_view, d->pal_bgra,
                         COLECOVDP_PAL_W, COLECOVDP_PAL_H);
...
static void layout(debugger *d)
{
     ...
     * table right. The views are drawn at 2x (1x for the palette strip). */
    {
        ...
        MoveWindow(d->view_pal, vx, vy + 392, COLECOVDP_PAL_W, COLECOVDP_PAL_H,
                   TRUE);
        vx += 524;
...
static LRESULT CALLBACK dbg_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_SIZE:
        ...
        return 0;
    case WM_TIMER:
        if (wp == TIMER_REFRESH && !colecodebug_is_paused(d->dbg)) {
            refresh_regs(d);
        ...
        return 0;
    case WM_DBG_STOPPED:
        on_stopped(d, (colecodebug_stop_reason)wp, (uint16_t)lp);
        return 0;
    case WM_DBG_ACCEPT:
        on_accept(d, (int)wp);
        return 0;
    ...
    case WM_COMMAND:
        ...
        case IDC_STEP_IN:   colecodebug_step_into(d->dbg); return 0;
        case IDC_STEP_OVER: colecodebug_step_over(d->dbg); return 0;
        case IDC_STEP_OUT:  colecodebug_step_out(d->dbg); return 0;
        case IDC_BP_CLEAR:
            colecodebug_bp_clear_all(d->dbg);
            refresh_bps(d);
            refresh_disasm(d);
            return 0;
        case IDC_TRACE:
            colecodebug_trace_enable(
                d->dbg,
    ...
    case WM_DESTROY:
        ...
        colecodebug_set_stop_callback(d->dbg, NULL, NULL);
        DeleteObject(d->mono);
        free(d);
        g_dbg = NULL;
        return 0;
...
void coleco_debugger_show(HWND parent, colecosession *session)
{
    ...
    wc.lpszClassName = "ColecoDebuggerWindow";
    RegisterClassA(&wc);
    ...
    wc.hInstance = inst;
    wc.hCursor = LoadCursor(NULL, IDC_ARROW);
    wc.lpszClassName = "ColecoDbgPixels";
    RegisterClassA(&wc);
    ...
    if (!d)
        return;
    d->session = session;
    d->dbg = colecosession_debugger(session);
    d->follow_pc = 1;
    d->mem_base = 0xFC30;
    g_dbg = d;

    d->hwnd = CreateWindowExA(0, "ColecoDebuggerWindow", "ColecoVision Debugger",
    ...
    colecodebug_set_stop_callback(d->dbg, stop_trampoline, d);
    SetTimer(d->hwnd, TIMER_REFRESH, 100, NULL);

    /* Dev affordance: COLECO_DEBUGGER_TAB=vdp|vram|trace picks the start
     * tab. */
    tab = getenv("COLECO_DEBUGGER_TAB");
    d->page = PAGE_CPU;
