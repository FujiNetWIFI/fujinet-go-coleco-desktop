 ...
 * VDP visualizers, over the shared colecodebug engine. The engine's stop
...
void stop_trampoline(void *ud, colecodebug_stop_reason reason, uint16_t pc)
{
...
void DebuggerWindow::showFor(QWidget *parent, colecosession *session)
{
    if (g_singleton) {
        g_singleton->raise();
...
DebuggerWindow::DebuggerWindow(QWidget *parent, colecosession *session)
    : QMainWindow(parent), m_session(session)
{
    m_dbg = colecosession_debugger(session);
    setWindowFlag(Qt::Window);
    setAttribute(Qt::WA_DeleteOnClose);
    setWindowTitle(QStringLiteral("ColecoVision Debugger"));
    resize(1180, 820);
    buildUi();

    connect(this, &DebuggerWindow::stopped, this, &DebuggerWindow::onStopped,
            Qt::QueuedConnection);
    colecodebug_set_stop_callback(m_dbg, stop_trampoline, this);

    m_tick = new QTimer(this);
    connect(m_tick, &QTimer::timeout, this, [this] {
        if (!colecodebug_is_paused(m_dbg)) {
            refreshVdp();
            refreshRegs();
        }
    });
...
{
    colecodebug_set_stop_callback(m_dbg, nullptr, nullptr);
}
...
void DebuggerWindow::buildUi()
{
    ...
    connect(stepOut, &QPushButton::clicked, this,
            [this] { colecodebug_step_out(m_dbg); });
    ...
    connect(trace, &QCheckBox::toggled, this, [this](bool on) {
        colecodebug_trace_enable(m_dbg, on ? 1 : 0);
    });
    tb->addWidget(trace);
    ...
    connect(m_disasm, &QPlainTextEdit::cursorPositionChanged, this, [this] {
        ...
        if (ok && m_disasm->hasFocus()) {
            colecodebug_bp_toggle(m_dbg, addr);
            refreshDisasm();
            refreshBps();
        }
    });
    ...
    for (int i = 0; i < 8; i++) {
        ...
        connect(m_regEdit[i], &QLineEdit::returnPressed, this, [this, idx] {
            ...
            if (!ok)
                return;
            adamcore_z80_regs r;
            colecodebug_get_regs(m_dbg, &r);
            regSet(r, idx, v);
            colecodebug_set_regs(m_dbg, &r);
            refreshRegs();
            refreshDisasm();
        });
    ...
    connect(bpClear, &QPushButton::clicked, this, [this] {
        colecodebug_bp_clear_all(m_dbg);
        refreshBps();
        refreshDisasm();
    });
    ...
    m_pal = new QLabel;
    m_pal->setFixedSize(COLECOVDP_PAL_W, COLECOVDP_PAL_H);
...
bool DebuggerWindow::parseAddr(const QString &text, quint16 *out)
{
    ...
    uint16_t addr;
    if (colecodebug_symbol_find(m_dbg, text.trimmed().toUtf8().constData(),
                              &addr)) {
        *out = addr;
        return true;
    }
...
{
    if (colecodebug_is_paused(m_dbg)) {
        colecodebug_resume(m_dbg);
        m_status->setText(QStringLiteral("Running"));
        refreshAll();
    } else {
        colecodebug_pause(m_dbg);
    }
}
...
void DebuggerWindow::onStopped(int reason, quint16 pc)
{
    static const char *const names[] = {"paused", "breakpoint", "step",
                                        "run-to"};
    uint16_t off = 0;
    const char *sym = colecodebug_symbol_at(m_dbg, pc, &off);
...
void DebuggerWindow::refreshAll()
{
    ...
    refreshTrace();
    refreshVdp();
    m_pauseBtn->setText(colecodebug_is_paused(m_dbg)
...
{
    colecodasm_line lines[kDisasmLines];
    adamcore_z80_regs r;
    colecodebug_get_regs(m_dbg, &r);
    if (m_followPc)
        m_disasmBase = r.pc;
    ...
    const int n =
        colecodebug_disassemble(m_dbg, m_disasmBase, kDisasmLines, lines);
    QString text;
    for (int i = 0; i < n; i++) {
        if (lines[i].symbol)
            text += QStringLiteral("%1%2:\n")
        ...
        text += QStringLiteral("%1%2%3  %4 %5\n")
                    .arg(colecodebug_bp_is_set(m_dbg, lines[i].addr)
                             ? QLatin1Char('*')
                             : QLatin1Char(' '))
...
void DebuggerWindow::refreshMem()
{
    ...
    colecodebug_read_mem(m_dbg, m_memBase, data, sizeof(data));
    QString text;
...
{
    uint16_t bps[128];
    const int n = colecodebug_bp_list(m_dbg, bps, 128);
    QString text = n ? QString() : QStringLiteral("(no breakpoints)");
    for (int i = 0; i < n; i++) {
        uint16_t off = 0;
        const char *sym = colecodebug_symbol_at(m_dbg, bps[i], &off);
...
void DebuggerWindow::refreshTrace()
{
    ...
    const int n = colecodebug_trace_read(m_dbg, entries, 256);
...
void DebuggerWindow::refreshVdp()
{
    ...
    m_pal->setPixmap(QPixmap::fromImage(QImage(pal, COLECOVDP_PAL_W,
                                               COLECOVDP_PAL_H,
                                               COLECOVDP_PAL_W * 4,
    ...
    colecovdp_format_state(&snap, state, int(sizeof(state)));
    m_vdpState->setPlainText(QString::fromLatin1(state));

    colecovdp_format_hex(&snap, m_vramBase, kVramRows, state,
                       int(sizeof(state)));
...
void DebuggerWindow::vramSubmit()
{
    ...
    quint16 addr = 0;
    const int n = colecovdp_parse_poke(m_vramEntry->text().toUtf8().constData(),
                                     &addr, bytes, kPokeMax);
    if (n < 0) {
    ...
    if (n > 0) {
        colecodebug_vdp_write(m_dbg, addr, bytes, n);
