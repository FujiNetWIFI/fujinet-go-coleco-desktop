...
#ifndef COLECO_DYNLIB_H
#define COLECO_DYNLIB_H
...
typedef HMODULE coleco_dynlib;

static inline coleco_dynlib coleco_dynlib_open(const char *path)
{
...
static inline void *coleco_dynlib_sym(coleco_dynlib h, const char *name)
{
    return (void *)(uintptr_t)GetProcAddress(h, name);
}

static inline const char *coleco_dynlib_error(char *buf, int buflen)
{
