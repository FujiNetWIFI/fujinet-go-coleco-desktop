/*
 * Symbol tables for the colecodebug engine. Parses the ".sym" format
...
static void table_add(sym_table *t, uint16_t addr, const char *name)
{
    ...
    /* A symbol longer than the entry is truncated on purpose -- a clipped
     * label in a disassembly is fine, refusing to load the table is not. */
    snprintf(t->entries[t->count].name, sizeof(t->entries[t->count].name),
             "%.*s", (int)sizeof(t->entries[t->count].name) - 1, name);
    t->count++;
}
