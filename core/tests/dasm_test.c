 ...
 *
 * z80dasm.c is a FRESH implementation written from that manual rather than
 * adapted from an existing disassembler -- ADAMEm's is non-commercial and
 * GPL-incompatible (COMPLIANCE.md) -- so it has no upstream to inherit
 * correctness from. This file is that correctness argument.
 *
...
int main(void)
{
    ...
    check(0, C(0x76), "HALT", 1, COLECODASM_HALT, -1);
    ...
    check(0, C(0xE9), "JP (HL)", 1, COLECODASM_JUMP, -1);
    check(0, C(0xC9), "RET", 1, COLECODASM_RET, -1);
    check(0, C(0xC0), "RET NZ", 1, COLECODASM_RET | COLECODASM_COND, -1);
    check(0, C(0xC7), "RST $00", 1, COLECODASM_CALL, 0x0000);
    check(0, C(0xFF), "RST $38", 1, COLECODASM_CALL, 0x0038);

    check(0, C(0xC3, 0x34, 0x12), "JP $1234", 3, COLECODASM_JUMP, 0x1234);
    check(0, C(0xFA, 0x00, 0x80), "JP M,$8000", 3,
          COLECODASM_JUMP | COLECODASM_COND, 0x8000);
    check(0, C(0xCD, 0xCD, 0xAB), "CALL $ABCD", 3, COLECODASM_CALL, 0xABCD);
    check(0, C(0xD4, 0x00, 0x90), "CALL NC,$9000", 3,
          COLECODASM_CALL | COLECODASM_COND, 0x9000);

    /* Relative targets from a nonzero base. */
    check(0x0100, C(0x18, 0xFE), "JR $0100", 2,
          COLECODASM_JUMP | COLECODASM_RELATIVE, 0x0100);
    check(0x0100, C(0x20, 0x05), "JR NZ,$0107", 2,
          COLECODASM_JUMP | COLECODASM_RELATIVE | COLECODASM_COND, 0x0107);
    ...
    check(0, C(0xED, 0x4D), "RETI", 2, COLECODASM_RET, -1);
    check(0, C(0xED, 0x45), "RETN", 2, COLECODASM_RET, -1);
    check(0, C(0xED, 0xB0), "LDIR", 2, COLECODASM_BLOCK, -1);
    ...
    check(0, C(0xDD, 0xE9), "JP (IX)", 2, COLECODASM_JUMP, -1);
